...
- **全チャンネル既定値**: オプション画面でVideo / Live別にLUFS自動適用の初期値を設定可能。個別設定済みチャンネルを変更せず、未設定チャンネルだけが継承
...
- **フォールバック表示**: LUFS未検出時はポップアップのCurrentにFallbackを表示し、Saved Channelsでは`Auto (70%)`の形式で種別ごとのフォールバック値を確認可能
