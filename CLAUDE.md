...
- **fold が失敗した間は旧規則で解決する**: 成功を `storageMigrated` で持ち、false の間は `resolveAutoApplySetting(..., unmigrated)` が「保存済みゲイン = Auto OFF」の旧規則を使う (options.js の一覧表示も同じ)。失敗を成功扱いにすると、印もフラグも保存されないまま Auto が手動ゲインを上書きする。`triggerApply()` は `storageMigrated` が false の間 fold を再試行するため、一時的な書き込み失敗は次の apply で回復する
