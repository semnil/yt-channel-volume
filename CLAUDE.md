...
- **A page fetch nothing handles**: Chrome reports an unhandled rejection from a page request against the `window.fetch` wrapper the request went through, and records `Uncaught (in promise) TypeError: Failed to fetch` in the extension's error list. A request made while `isWatchPage()` holds gets a rejection handler that records its reason in a `WeakSet` when the reason is a `TypeError` reading `Failed to fetch`, and the bridge's `unhandledrejection` listener prevents the default for a reason in that set alone. The page's own promise rejected with the same error, a request made on another page (whatever page it fails on), and a request that fails with anything else are left as they are. The handler holds the request's promise as handled, so a request made on a watch page that the page leaves with no handler at all raises no unhandled rejection, whatever it fails with
