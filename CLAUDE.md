...
- **Filling in isLiveNow**: `_capturedResp`'s `isLiveNow` is fixed as of page load, so the request handler asks the page's own responses whether the stream has started (this covers the waiting → stream start transition). Both are asked — `ytd-watch-flexy` holds the response the page was built with and can still say waiting, while `movie_player.getPlayerResponse()` holds the one it is running now — and either saying `isLive` is taken, after each is held to the video the URL names. content.js's `forceDetect` (on popup open) asks the bridge again, and where the response updates `currentIsLiveNow` the popup is notified through `stateChanged`
